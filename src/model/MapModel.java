...
public abstract class MapModel<E> { //collection of positions
	
	 protected Position[][] positions; //first[] = row second[]=column //x->left to right y-> up to down
	 ...
	 public void addEnemy(int x,int y) {
		 this.positions[x][y].setEnemy(true);
	 }
	 
	 public Vector<Position> getEnemy(){
		 Vector<Position> vp = new Vector<Position>();
		 for(int i = 0;i<1200;i+=50) {
			 for(int j = 0 ; j<800;j+=50) {
				 if(!positions[i][j].hasEnemy()) {vp.add(positions[i][j]);}
			 }
		 }
		 return vp;
	 }
	 ...
	 protected boolean checkWall(int x,int y) {
		 return positions[x][y].isWalkable();
	 }
}
